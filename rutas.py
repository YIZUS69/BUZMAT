...
def buscar_itinerario_con_caminata(origen: str, destino_real: str, radio_km: float = 2.5, max_candidatos: int = 15):
    """
    Igual que buscar_itinerario, pero si no hay NINGUNA conexión en bus
    entre origen y destino_real, busca la parada más cercana al destino
    real (dentro de `radio_km`) a la que SÍ se pueda llegar en bus, para
    sugerir "camina el último tramo".

    Devuelve un dict:
        {
            "tramos": [...] o None,
            "parada_bajada": nombre de la parada donde te bajas del bus,
            "caminata_km": distancia a pie desde esa parada hasta el
                           destino real (0.0 si hay conexión directa en bus),
        }
    o None si ni siquiera caminando desde alguna parada cercana se
    encuentra conexión.
    """
    tramos = buscar_itinerario(origen, destino_real)
    if tramos is not None:
        return {"tramos": tramos, "parada_bajada": destino_real, "caminata_km": 0.0}

    # Sin conexión directa en bus: probamos con paradas cercanas al
    # destino real, empezando por la más cercana.
    destino_datos = PARADAS[destino_real]
    candidatos = []
    for nombre, datos in PARADAS.items():
        if nombre == destino_real:
            continue
        d = haversine_km(destino_datos["lat"], destino_datos["lon"], datos["lat"], datos["lon"])
        if d <= radio_km:
            candidatos.append((d, nombre))
    candidatos.sort()

    for distancia, candidato in candidatos[:max_candidatos]:
        tramos_candidato = buscar_itinerario(origen, candidato)
        if tramos_candidato is not None:
            return {"tramos": tramos_candidato, "parada_bajada": candidato, "caminata_km": distancia}

    return None


def formatear_itinerario_con_caminata(resultado) -> str:
    """Formatea el resultado de buscar_itinerario_con_caminata en texto
    legible, incluyendo la nota de caminata si aplica."""
    if resultado is None:
        return (
            "No encontré ninguna forma de llegar, ni en bus ni caminando "
            "hasta una parada cercana, con las rutas registradas actualmente."
        )

    texto = formatear_itinerario(resultado["tramos"])

    if resultado["caminata_km"] > 0:
        texto += (
            f"\n🚶 Ningún bus llega directo a tu destino final: bájate en "
            f"{resultado['parada_bajada']} y camina aproximadamente "
            f"{resultado['caminata_km']:.2f} km hasta tu destino."
        )
