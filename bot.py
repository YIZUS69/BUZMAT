...
from rutas import buscar_itinerario_con_caminata, formatear_itinerario_con_caminata
...
async def recibir_destino(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if rutas_comunes:
        # Hay conexión directa: mostramos TODAS las rutas comunes.
        texto_buses = "Ruta directa: " + ", ".join(rutas_comunes)
    else:
        # No hay conexión directa: buscamos el itinerario más corto
        # posible con transbordo(s), y si ni así hay conexión, sugerimos
        # bajarse en la parada más cercana y caminar el resto.
        resultado = buscar_itinerario_con_caminata(origen, destino)
        texto_buses = formatear_itinerario_con_caminata(resultado)
    ...
    mensaje = (
        f"📍 *Origen:* {origen}\n"
        f"🏁 *Destino:* {destino}\n"
        f"🚌 *Cómo llegar:* {texto_buses}\n"
        f"💵 *Costo máximo del pasaje:* {PRECIO_PASAJE_USD:.2f} USD por cada bus tomado "
        f"(~ {costo_bs:.2f} Bs. c/u) según tasa oficial BCV."
    )
